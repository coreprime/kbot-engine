...
// tarball contains exactly the allow-listed files and none of its bytes look
// like a credential. The extracted tree is left behind for external scanners
// (trufflehog / trivy) to sweep before `npm publish <tarball>` ships the
// exact artifact that was verified.
//
...
// prepack (the wasm build) writes to stdout ahead of npm's JSON; parse from
...
for (const f of files) {
  ...
  if (!allowed.has(f.path)) fail(`unexpected file in tarball: ${f.path}`)
